...
export const addWorkouts = async ({
  ...
  const workoutInstanceIds: WorkoutEvent[] = []

  await db.transaction(async tx => {
    for (const templateId of workoutTemplateIds) {
      ...
      workoutInstanceIds.push({
        id: workoutInstanceId,
        name: workoutTemplateRes[0].name,
        date: date
      })

      const exercises = await tx.select({
