...
export function EntitesByDate({
  ...
  const createWeekBucket = () => {
    ...
    for (const entity of entities) {
      const bucket = Math.floor((entity.date.getTime() - lowerBoundry.getTime()) / 304_800_000)
  ...
  const createMonthBucket = () => {
    ...
    for (const entity of entities) {
      if (firstMonth <= entity.date && entity.date < firstDayAfterLastMonth) {
        let month = entity.date.getMonth()
    ...
    <div className="w-full">
      <div className="w-full flex border-b divide-x">
        <div className="flex-1 py-2 px-2 flex items-center text-sm">
          <p className="text-muted-foreground">Number of {entityName} by {freq}</p>
        </div>
        <div 
          className={cn(
